...
def eon_dataframe(df):
    ...
    columns_to_drop = [col for col in df_filtered.columns if col.startswith("S") or col in ["OBIS", "Szorzo", "Mertekegys"]]
    ...
    # Melt the dataframe
    melted_df = df_filtered.melt(id_vars=['MP', 'Datum'], 
                                 value_vars=[col for col in df_filtered.columns if col.startswith("A") and int(col[1:3]) <= 23 and int(col[3:]) < 4],
                                 var_name='Time_Slice', 
                                 value_name='Value')
    ...
    def get_time_interval(row):
        ...
        start_time = date.replace(hour=hour, minute=minute, second=0, microsecond=0).strftime('%H:%M')
        end_time = (date + pd.Timedelta(minutes=15)).replace(hour=hour, minute=minute, second=0, microsecond=0).strftime('%H:%M')
    ...
    reshaped_df = melted_df.pivot_table(index='Időszeletek', columns='MP', values='Value', aggfunc='sum').reset_index()
